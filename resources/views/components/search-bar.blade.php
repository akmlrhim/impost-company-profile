  ...
  <form method="GET" action="{{ $searchRoute }}" class="w-full sm:max-w-sm">
    <div class="relative flex">
      ...
      <input type="text" name="{{ $name }}" value="{{ request($name) }}" placeholder="{{ $placeholder }}"
        autocomplete="off"
        class="w-full rounded-md border border-gray-300 bg-white px-4 py-2.5 pr-20 text-sm text-gray-700 placeholder-gray-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition">

      <span class="absolute inset-y-0 right-10 flex items-center text-gray-400">
