  ...
  <main class="pt-28 pb-16 bg-impost-fifth antialiased">
    <div class="px-4 mx-auto max-w-7xl">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              ...
              <a href="/articles" class="ml-1 text-sm font-medium text-gray-400 hover:text-white md:ml-2">Articles</a>
            </div>
          </li>
          <li aria-current="page">
            <div class="flex items-center">
              ...
              <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">{{ Str::limit($article->title, 30) }}</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <article class="mx-auto w-full max-w-4xl">
        ...
        <header class="mb-8">
          <h1 class="mb-6 text-4xl font-bold leading-tight text-white">
            {{ $article->title }}
          </h1>
          ...
          <section class="mt-16" id="comment-section">
            <div class="mb-8">
              <h2 class="text-2xl font-bold text-white">
                Komentar <span class="text-lg font-normal text-gray-500">({{ $article->comments->count() }})</span>
              </h2>
            </div>
            ...
            {{-- form komentar  --}}
            <div class="bg-gray-900 border border-gray-800 rounded-md p-6 mb-8">
              <h3 class="text-lg font-semibold text-white mb-4">Tulis Komentar</h3>
              ...
              <form class="space-y-4" action="{{ route('article.comment') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label for="fullname" class="block mb-2 text-sm font-medium text-gray-300">
                      Nama Lengkap <span class="text-red-400">*</span>
                    </label>
                    <input type="text" id="fullname" name="fullname"
                      class="w-full px-4 py-2.5 text-sm text-white bg-gray-950 border border-gray-800 rounded-md focus:ring-1 focus:ring-primary-500 focus:border-primary-500 placeholder-gray-500"
                ...
                <div>
                  <label for="comment" class="block mb-2 text-sm font-medium text-gray-300">
                    Komentar <span class="text-red-400">*</span>
                  </label>
                  <textarea id="comment" name="comment" rows="5"
                    class="w-full px-4 py-2.5 text-sm text-white bg-gray-950 border border-gray-800 rounded-md focus:ring-1 focus:ring-primary-500 focus:border-primary-500 placeholder-gray-500 resize-none"
                ...
                <div class="flex items-center justify-between pt-2">
                  <button type="submit"
                    class="py-2.5 px-6 text-sm font-semibold text-white bg-primary-600 rounded-md hover:bg-primary-700 focus:ring-2 focus:ring-primary-500">
                    Kirim Komentar
                  </button>

                  <button type="reset" class="text-sm text-gray-500 hover:text-white">
                    Reset Form
                  </button>
                </div>
              </form>
            ...
            <div x-data="{ showAll: false, limit: 3 }" class="space-y-4">
              ...
              @forelse ($article->comments as $index => $c)
                <article x-show="showAll || {{ $index }} < limit" x-transition
                  class="bg-gray-900 border border-gray-800 rounded-md p-5">
