...
@section('content')
  ...
  <x-search-bar search-route="{{ route('study-case.index') }}" add-route="{{ route('study-case.create') }}"
    add-label="Tambah Study Case" />

  <div class="bg-neutral-primary-soft rounded-base border border-default">
    <div class="p-4">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        @forelse ($studyCases as $studyCase)
          <div class="bg-white rounded-sm shadow-sm overflow-hidden flex flex-col h-full">

            <div class="relative h-48 bg-gray-100">

              @if ($studyCase->cover_path)
                <img src="{{ asset('storage/' . $studyCase->cover_path) }}" alt="{{ $studyCase->name }}" loading="lazy"
            ...
            <div class="p-5 flex flex-col flex-1">
              ...
              <div class="mt-auto flex items-center gap-2">
                <a href="{{ route('study-case.edit', $studyCase) }}"
                ...
                <x-confirm-delete :action="route('study-case.destroy', $studyCase)" label="Hapus" />
              </div>
            </div>
          </div>
        @empty
          <div class="col-span-full bg-white p-12">
            <div class="flex flex-col items-center justify-center text-center">
              ...
              <p class="text-gray-500 text-md font-medium">Tidak ada Study Case ditemukan</p>
              <p class="text-gray-400 text-sm mt-1">Mulai dengan menambahkan Study Case baru</p>
            </div>
          </div>
        @endforelse
      </div>
